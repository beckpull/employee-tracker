...
class CLI {
  ...
  async viewAllEmployees() {
    console.log(`\nEmployees:\n`);
    const sql = `SELECT e.id AS employee_id, CONCAT(e.first_name, ' ', e.last_name) AS employee, r.title AS job_title, d.dept_name AS department, r.salary, CONCAT(m.first_name, ' ', m.last_name) AS manager_name FROM employees e JOIN roles r ON e.role_id = r.id JOIN departments d ON r.dept_id = d.id LEFT JOIN employees m ON e.manager_id = m.id;`;
  ...
  async viewEmpsByDept() {
    try {
      ...
      await queryObj.selectQuery(sql, param);
      this.run();
    } catch (err) {
      console.error(err);
    }
  ...
  async updateEmpRole() {
    try {
      ...
      await queryObj.editQuery5(sql, params);
  
      const sql2 = `SELECT CONCAT(employees.first_name, ' ', employees.last_name) AS all_employees, roles.title FROM employees JOIN roles ON employees.role_id = roles.id WHERE employees.id = ${employee};`;
      await this.selectQuery(sql2);
      this.run();
    } catch (err) {
      console.error(err);
    }
  ...
  async removeEmp() {
    try {
      ...
      if (confirm) {
        ...
        const data = await this.selectQuery(sql);
        console.log(`\n${JSON.stringify(data.rowCount)} employee ${data.command.toLowerCase()}d.\n`);
  ...
  async addDept() {
    try {
      ...
      const params = dept;
      const data = await this.editQuery(sql, params);
      ...
      this.run(); // Run after selectQuery is completed
    } catch (err) {
      console.log(err);
    }
  }
  ...
  viewRoles() {
    console.log(`\nRoles:\n`);
    const sql = `SELECT roles.id AS role_id, roles.title, roles.salary, departments.dept_name AS department FROM roles JOIN departments ON roles.dept_id = departments.id;`;
    this.selectQuery(sql);
    setTimeout(() => this.run(), 1000);
  }
  ...
  async updateSalary() {
    ...
    inquirer.prompt([
      ...
      .then((res) => {
        ...
        const queryObj = {
          async editQuery3(sql, params) {
            try {
              ...
              return data; // Return the result of the salary update operation
            } catch (err) {
              throw new Error(err);
            }
          }
        ...
        return queryObj.editQuery3(sql, params); // Return the promise chain
      })
      .then(() => {
        const sql2 = `SELECT roles.title, roles.salary FROM roles;`;
        return this.selectQuery(sql2); // Return the promise chain
      })
      .then(() => {
        setTimeout(() => this.run(), 1000);
      })
  ...
  async viewManagers() {
    try {
        ...
        console.error("Error occurred while viewing managers:", error);
    }
}
...
async viewEmpByManager() {
  try {
      ...
      console.table(result); // Assuming this will print the results in a table format
      setTimeout(() => this.run(), 1000);
  } catch (error) {
    console.error("Error occurred while viewing employees by manager:", error);
}
}
