...
class CLI {
  ...
  async addRole() {
    ...
    inquirer
      ...
      .then((res) => {
        ...
        return this.editQuery(sql2, params2);
      })
      .then((data) => {
        // console.log(`Data: ${JSON.stringify(data)}`);
  ...
  async updateSalary() {
      ...
      .then((res) => {
        ...
        const sql = `UPDATE roles SET roles.salary= $2 WHERE roles.id = $1`;
        console.log(`SQL: ${sql} PARAMS: ${params}`);
        queryObj.editQuery3(sql, params);
        console.log(role, salary);
        return role, salary;
      })
      .then((res) => {
        const { role, salary } = res;
        console.log(`\nNew Salary for ${role}: ${salary}.\n`);
